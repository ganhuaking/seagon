...
class Quotation
{
    ...
    public function __invoke(Request $request, Closure $next)
    {
        ...
        if ('師公語錄' === $text) {
            ...
            return new TextMessageBuilder($this->quotation->random($events)['text']);
        }
        ...
        if (preg_match('/^師公語錄(\d+)$/', $text, $match)) {
            ...
            return new TextMessageBuilder($this->quotation->get($events, $match[1])['text']);
        }
        ...
        if (preg_match('/^師公語錄話(.*)$/', $text, $match)) {
            ...
            return new TextMessageBuilder($this->quotation->find($events, $match[1])['text']);
        }
